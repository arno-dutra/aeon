...
class InceptionDecoderNetwork(BaseDeepNetwork):  # TODO @arno-dutra : En cours
    """InceptionTime Network.
        ...
        use_up_sampling     : bool or list of bool, default = True,
            conditioning wether or not to use up sampling
            layer in inception modules, if not a list,
            the same is used in all inception modules
        up_sampling_size       : int or list of int, default = 3,
            the size of the up sampling layer, if not a list,
            the same is used in all inception modules
    ...
    def __init__(
        ...
        nb_conv_per_layer=3,
        kernel_size=40,
        use_up_sampling=True,
        up_sampling_size=3,
        use_up_sampling_between_blocks=False,
        between_blocks_up_sampling_size=3,
        strides=1,
        dilation_rate=1,
        padding="same",
        activation="relu",
        ...
        self.use_up_sampling_between_blocks = use_up_sampling_between_blocks
        self.between_blocks_up_sampling_size = between_blocks_up_sampling_size
    ...
    def hybrid_layer(self, input_tensor, input_channels, kernel_sizes=None):  # TODO @arno-dutra : NotImplemented
    ...
    def _inception_module(
        ...
        # if use_up_sampling:
        #     up_sampling_1 = tf.keras.layers.UpSampling1D(
        #         size=up_sampling_size
        #     )(input_tensor)

        #     conv_up_sampling = tf.keras.layers.Conv1D(
        #         filters=nb_filters,
        #         kernel_size=1,
        #         padding=padding,
        #         activation="linear",
        ...
        #     conv_list.append(conv_up_sampling)
    ...
    def build_network(self, input_layer, **kwargs):
        ...
        if isinstance(self.use_up_sampling, list):
            self._use_up_sampling = self.use_up_sampling
        else:
            self._use_up_sampling = [self.use_up_sampling] * self.depth

        if isinstance(self.up_sampling_size, list):
            self._up_sampling_size = self.up_sampling_size
        else:
            self._up_sampling_size = [self.up_sampling_size] * self.depth

        if isinstance(self.use_up_sampling_between_blocks, list):
            self._use_up_sampling_between_blocks = self.use_up_sampling_between_blocks
        else:
            self._use_up_sampling_between_blocks = [self.use_up_sampling_between_blocks] * self.depth

        if isinstance(self.between_blocks_up_sampling_size, list):
            self._between_blocks_up_sampling_size = self.between_blocks_up_sampling_size
        else:
            self._between_blocks_up_sampling_size = [self.between_blocks_up_sampling_size] * self.depth
        ...
        for d in range(self.depth):
            ...
            x = self._inception_module(
                ...
                use_up_sampling=self._use_up_sampling[d],
                up_sampling_size=self._up_sampling_size[d],
            ...
            if self._use_up_sampling_between_blocks[d]:
                x = tf.keras.layers.UpSampling1D(
                    size=self._between_blocks_up_sampling_size[d]
                )(x)
